...
def train(args, env_sampler, predict_env, agent, env_pool, model_pool):
    ...
    for epoch_step in tqdm(range(args.num_epoch)):
        ...
        if epoch_step % eval_interval == 0:
            ...
            print("", flush=True)
            print(f'Epoch {epoch_step} Eval_Reward {rewards_avg:.2f} Eval_Cvar {cvar:.2f} Eval_Std {rewards_std:.2f} Normalized_Score {normalized_score:.2f}', flush=True)
            if args.wandb:
...
def main():
    ...
    args.save_dir = f'result-{args.algo}{args.comments}/{args.env}-CVaR{args.wcql_cvar}-{datetime.datetime.now().strftime("%m-%d-%H-%M-%S")}/seed-{run_name}'
