...
class CODAC_CVAR(object):
    def __init__(self, num_inputs, action_space,
        ...
        print("Init codac cvar ", codac_cvar)
        self.codac_cvar = codac_cvar
        self.gamma = gamma
        ...
        self.target_policy = GaussianPolicy(num_inputs, action_space.shape[0], hidden_size, action_space).to(
            self.device)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        if self.dist_penalty_type != 'none':
            ...
            curr_actions_tensor, curr_log_pis = self._get_policy_actions(state, num_actions=self.num_random, network=self.policy)
            new_curr_actions_tensor, new_log_pis = self._get_policy_actions(next_state, num_actions=self.num_random, network=self.policy)

            penalty_index = np.random.randint(int(self.codac_cvar*self.num_quantiles), self.num_quantiles)
